...
(function () {
  ...
  function renderLot(id) {
    ...
    if (del) del.addEventListener('click', function () {
      // по партии могут висеть чужие заказы и брони: удалить её молча —
      // значит оставить покупателя с заказом на то, чего больше нет
      var tied = orders().filter(function (o) {
        return o.lotId === id && (o.status === 'new' || o.status === 'confirmed');
      });
      var warn = 'Карточка «' + work.name + '» исчезнет с сайта вместе со ссылкой на неё. ' +
        'Если партия просто кончилась, лучше закрыть её: карточка останется в поиске.';
      if (tied.length) {
        warn = 'По этой партии есть незакрытые записи: ' + tied.length + ' шт. ' +
          'Они останутся у покупателей в кабинете, но вести будет некуда. ' +
          'Сначала закройте или отмените их в разделе «Заказы». ' + warn;
      }
      confirmDanger('Удалить партию?', warn,
        'Да, удалить', function () {
